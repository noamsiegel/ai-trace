---
id: COMPARISON
summary: "How agents-trace differs from agent session exporters, transcript viewers, observability traces, and build provenance systems"
---

# How agents-trace compares
...
artifacts. `agents-trace` sits downstream of local coding agents and upstream of PR
review: it turns local Claude Code/Codex JSONL into scrubbed, gitleaks-gated,
secret-gist evidence linked from a GitHub PR.

## At a glance

| Project | Core verb | Output | Loaded at | Composable with agents-trace? |
|---|---|---|---|---|
| **agents-trace** | attach | scrubbed Markdown gist linked from PR body | PR review time | — |
...
| [Codex CLI](https://github.com/openai/codex) | run/resume | local session JSONL under `~/.codex/sessions` | during Codex CLI use | yes — agents-trace already consumes Codex JSONL when cwd matches the repo |
| [codex-transcript-viewer](https://github.com/masonc15/codex-transcript-viewer) | render | single-file HTML transcript | after Codex sessions exist | yes — viewer UX is complementary to agents-trace's PR attachment flow |
| [Cline](https://github.com/cline/cline) | run/checkpoint | IDE/CLI sessions, checkpoints, plugin-defined logs | agent runtime | partial — Cline plugins could emit audit logs, but agents-trace owns PR linkage |
...
`agents-trace` owns review plumbing: scrub, neutralize, gitleaks-gate, create a
secret gist, and attach/update a PR marker.

**Compose**: a Goose source adapter would make sense only if Goose exports carry
enough repo cwd, timestamps, prompts, and file references to pass agents-trace's
...
`agents-trace` produces reviewer-visible evidence with public-repo safety checks and
...
**Difference**: Codex persists sessions so Codex can resume work. `agents-trace`
...
**Difference**: it is a viewer. `agents-trace` is a secure collection, scrubbing,
gitleaks, gist, and PR attachment workflow.

**Compose**: HTML review UX is complementary, but would be a product expansion.
Today agents-trace's Markdown gist is deliberately lightweight and review-oriented.
...
`agents-trace` is not an agent runtime; it is downstream PR review infrastructure.

**Compose**: a Cline plugin could emit source data for agents-trace, but that is a
future source-adapter question, not a reason to change agents-trace's core shape.
...
to observability backends. `agents-trace` reads local coding-agent transcripts and
...
agents-trace ever needs standard trace import/export. It should not replace the
...
**Difference**: this is supply-chain provenance. `agents-trace` does not attest that
a build artifact was produced by a workflow; it captures prompts and local AI
coding intent for human PR review.

**Compose**: not today. A future custom predicate could embed an agents-trace digest,
...
and process?" `agents-trace` answers "what AI session evidence should a reviewer see
next to this PR?"

**Compose**: not today. `agents-trace` stays focused on AI session evidence, not
build attestation.
...
That is intentionally narrow. `agents-trace` should stay narrow unless a new source
